...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const otherPropertyTypeSelect = document.getElementById('otherPropertyType');
    ...
    document.getElementById('addressForm').addEventListener('submit', function(event) {
        ...
        .then(data => {
          ...
          document.getElementById('results').innerHTML = `<pre>${JSON.stringify(requiredData, null, 2)}</pre>`;
        })
        .catch(error => {
    ...
    searchButton.addEventListener('click', function(event) {
        ...
        const resultData = {
            ...
            propertyType: selectedPropertyType,  // The property type selected from the dropdown
        };

        // Display the result data (or you can process it as needed)
        resultContainer.textContent = JSON.stringify(resultData, null, 2);
    });
});
